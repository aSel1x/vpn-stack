...
### Variant B — SSH front (needed for iOS "SSH-over-DNSTT" apps)

Mobile apps (HTTP Injector, AnyBridge) run SSH *on top* of the tunnel, so dnstt hands off to sshd — set its last arg to `127.0.0.1:22`. Give those apps a locked-down, **non-root** login that can only forward to the SOCKS exit:

```bash
useradd -m -s /bin/bash tunuser && passwd tunuser
```

`/etc/ssh/sshd_config.d/60-tunuser.conf`:
...
Match User tunuser
    ...
    PermitOpen 127.0.0.1:1080 1.1.1.1:853   # SOCKS exit + the app's DoT resolver
```
...
> To avoid a host account entirely, run the sshd itself in a container and forward dnstt to it — same shape, no host user.
...
*This deployment: zone `tun.example.net`, server pubkey `7cccbd36c7cdb94eb07d0d120ff751959b7e7591e25afaa6f27281c6e4518566`; exit is Variant B (dnstt → host sshd `pentest-dnstt` → microsocks `127.0.0.1:7300`).*
