...
- allows exactly one user, password auth, `PermitOpen 127.0.0.1:7300` and nothing else;
...
Verified: a forward to `127.0.0.1:7300` carries traffic (SOCKS5 answers `05 00`); a forward to anything else is refused with *administratively prohibited*.
