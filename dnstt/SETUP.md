...
The username is `dnstt` and the password is generated by `vpnctl bootstrap`. Get both, with the zone and pubkey, from:

```bash
./vpn user export <name> --protocol dnstt
```

`PERMIT_OPEN` defaults to `127.0.0.1:7300 1.1.1.1:853` — the SOCKS exit **and the app's DoT resolver**. That second entry is not optional. HTTP Injector and AnyBridge resolve names over DNS-over-TLS *through the tunnel* before they open anything else, so with only the SOCKS exit listed every session dies at its first lookup, having never touched the SOCKS at all:
...
Verified: a forward to `127.0.0.1:7300` carries traffic (SOCKS5 answers `05 00`); a forward to a destination not in the list is refused with *administratively prohibited*.

If the app then asks for other destinations, they will appear in `docker logs dnstt-sshd` the same way — widen `PERMIT_OPEN` from what the log actually shows, rather than opening it up front.
