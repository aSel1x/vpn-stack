...
The app takes the other route: it has no checkout to push, so it clones the public repo onto the box. That is a supply-chain decision and is written down as one — an app-provisioned server executes whatever is on `main` at that instant, unpinned and unsigned.
