...
`provision` and `control` are pure Dart and unit-tested without a server: they build command
lines and parse JSON, and the SSH transport is injected. `tunnel` is the only layer that needs
platform code, and it is where the real work is.
